...
class DragonBonesFactory extends BaseClass{
     ...
     * @needCache
     */
    public makeArmature(name:string, fromDragonBonesDataName?:string, playSpeed:number = 1,needCache:boolean =false):DragonBonesArmature{
        var armature:dragonBones.FastArmature = this.factory.buildFastArmature(name, fromDragonBonesDataName);//buildArmature
        if(armature == null){
            egret.log("不存在Armature： "+name);
            return null;
        }
        ...
        if(needCache)
            result.getArmature().enableAnimationCache(30);
        return result;
    }
    /**
     * 创建一个动画（急速模式）
     ...
     * @returns {Armature}
     */
    public makeFastArmature(name:string, fromDragonBonesDataName?:string, playSpeed:number = 1,needCache:boolean =false):DragonBonesArmature {
